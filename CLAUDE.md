...
data/guestbook.json, data/reactions.json  # Visitor-submitted data — gitignored, created on first write
...
- **Guestbook and post reactions:** Two public write surfaces sharing `src/lib/jsonStore.ts` (per-file mutex + `tmp`/`rename` atomic write over `./data`, the docker-compose volume mount). `/guestbook` is SSR: `guestbook.astro` imports `getEntries()` **directly** rather than fetching its own `/api/guestbook` — an SSR self-fetch would hit the container's internal origin, not the canonical one (the same class of problem `src/middleware.ts` documents). Blog `[slug]` pages are prerendered, so `PostReactions.tsx` instead fetches counts on mount. Every reader tolerates a missing file and returns empty — the data files don't exist until the first write, and they're gitignored so visitor content never lands in a commit.
