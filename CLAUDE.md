...
Personal portfolio, blog, and games hub at **korikosmos.dev**. Built with Astro 5 (SSR mode) using the Node standalone adapter, with React islands (`@astrojs/react`) for components and interactivity, styled with Tailwind CSS 3 + DaisyUI 4.
...
- **Styling:** Tailwind CSS 3.3 + DaisyUI 4.12 (5 custom themes: dark, light, forest, spider-man, batman)
...
- `npm run gen:retro` — redraw the retro skin's graphics into `public/retro/` (output is committed)
- `npm run gen:og` — redraw the OG card artwork into `src/assets/og/` (output is committed)
...
├── assets/og/     # Generated OG card artwork (starfields + site mark) — see scripts/gen-og-assets.mjs
│                  #   Build-time only; never served, so it must not live under public/og/
...
│                  #   RetroLayout.astro (Web 1.0 shell), SeoHead.astro (meta for both)
...
│   ├── theme.ts          # modern theme allowlist + kk-theme cookie + getTheme()/persistTheme()
...
- **Theme system:** DaisyUI themes configured in `tailwind.config.mjs` with `data-theme` attribute on `<html>`. Five themes: dark (default), light, forest, spider-man, batman. **`src/lib/theme.ts` is the source of truth** for the list, the default, and the cookie; a new theme needs an entry in `THEMES` there, an entry in `tailwind.config.mjs`, a critical-background line in `ModernLayout.astro`'s inline `<style>`, a `ThemeBar.tsx` swatch, and a `CommandPalette.tsx` theme action. Custom `--surface` and `--accent-dark` CSS vars extend DaisyUI for specific utilities. The choice is stored in **both** `localStorage` (read by the navigation reconciler; `ThemeBar` observes the displayed `data-theme` to highlight its active swatch) and the `kk-theme` cookie — always write through `persistTheme()`, never `localStorage.setItem` alone, or the next server render reverts to the default. No theme provider/context. Theme switching animates via a `startViewTransition` circular reveal (CSS hooks under `html[data-theme-switching]` in `global.css`), with a CSS color-fade fallback (`data-theme-fade`) and instant switch under reduced motion.
- **View transitions / no-FOUC:** `ModernLayout.astro` uses Astro's `<ClientRouter />` for SPA-style navigation. Anti-flash relies on four head pieces: **`data-theme` rendered server-side on `<html>`** from the `kk-theme` cookie (via `locals.theme`), critical inline `<style>` with per-theme `html` backgrounds (correct first paint before the stylesheet loads), an inline script that reconciles the rendered theme with `localStorage` and re-applies it on `astro:after-swap` (the router resets `<html>` attributes on every swap, and a page prefetched before a theme change carries the stale one), and page animations in `global.css` (`vt-page-out`/`vt-page-in` on the root group). The server-rendered attribute is the load-bearing part: **an inline script is not guaranteed to run before paint.** Cloudflare Rocket Loader rewrites inline `<script type>` to a token MIME type and replays it after load, which is exactly how the flash-of-blue bug came back — the pre-paint scripts in both shells carry `data-cfasync="false"` to opt out of it, but only the SSR attribute is immune to a CSP or a future proxy doing the same. `localStorage` deliberately wins over the rendered attribute in that script, so a theme changed after a page was prefetched still applies. Header/footer have `transition:name` so they stay static during navigation; `ThemeBar`/`OnekoToggle` use `transition:persist`. The oneko cat is mounted *inside* the persisted OnekoToggle island (oneko.js appends next to its own script tag) so it survives navigations; islands that attach global listeners or rAF loops must clean up on unmount since navigation no longer reloads the page (Tetris does this via `game.running = false` + `removeEventListener`).
...
- **OG / social images:** `src/pages/og/[...route].ts` uses `astro-og-canvas` (prerendered) to generate cards for the site default (`/og/site.png`), every blog post (`/og/blog/<slug>.png`), and every project (`/og/portfolio/<slug>.png`). `Layout.astro` falls back to `/og/site.png` when no `image` prop is passed; blog/portfolio `[slug]` pages pass their generated card. Fonts are vendored in `src/fonts/` so builds don't hit a font CDN; note the 800-weight TTF registers as the separate family `'Noto Sans ExtraBold'`, not as a weight of `'Noto Sans'`. New content collection entries get OG cards automatically.
  - Cards are drawn over a **constellation starfield** committed to `src/assets/og/`, generated by `scripts/gen-og-assets.mjs` (`npm run gen:og`) — same arrangement as the retro assets, so the build never runs the script. Three variants share one PRNG seed (identical geometry) and differ only in accent, keyed off the `pages` key in `getImageOptions`: site = indigo, blog = sky, portfolio = violet. Output is PNG because CanvasKit's `MakeImageFromEncoded` **has no SVG decoder**; the artwork is composed as SVG for legibility and rasterised with `sharp` at script time. The emitted SVG must contain no `<text>` — librsvg would resolve fonts against the system, which is what vendoring the TTFs exists to avoid. The generator is seeded so re-running is a no-op in the working tree.
  - astro-og-canvas draws background → border → `bgImage`, so the starfield PNG carries alpha and the gradient shows through it. It also **clamps the text block to a narrow band below the logo** (see its `minTop`/`maxTop`); with no logo the title is pinned at `padding` and the bottom of the card is dead space, which is why the card mounts the site mark. Bottom-aligning the text is not reachable through this library's options.
- **Link-preview metadata:** `src/layouts/SeoHead.astro` emits the description/canonical/icon and all Open Graph + Twitter tags for **both** shells — edit there, not in a layout, or the two skins drift. It is output-only (props in, `<meta>`/`<link>` out); each shell keeps its own `<title>`, critical CSS, and scripts. `theme-color` drives Discord's embed stripe and mobile browser chrome, and is a fixed indigo because the crawler that reads it has no theme cookie. `og:title` deliberately keeps the full `{page} | {site}` string even though `og:site_name` is also set — Discord shows site name separately, but several clients ignore it and a bare title loses context there. `og:image:width`/`height` are declared so clients pick the large-image layout before fetching. Pages pass `ogType`/`publishedTime`/`modifiedTime` through `Layout.astro` for `article` metadata; only `blog/[slug]` does so far.
...
- **Skin resolution:** `src/lib/skin.ts` defines the cookie and `getSkin`/`isRetro`. `src/middleware.ts` resolves it into `Astro.locals.skin` before anything renders (and resolves `locals.theme` the same way, and bumps the hit counter there, since setting cookies from layout frontmatter races the streamed response). `App.Locals` is declared in `src/env.d.ts`.
