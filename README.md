...
- **Smooth theme reveals**: I keep storage writes outside the browser's snapshot pause and change the selected swatch with the page colours. Swatches and the command palette share one coordinator: during a reveal, my latest choice waits for it to finish before changing the page or saved preference. The circular reveal keeps its original timing, handles interruptions and respects reduced motion; browsers without view transitions get a colour fade.
