...
## Notes
- I keep palette and swatch choices in the same theme coordinator, scoped to the document rather than either island's mount. During a circular reveal I retain only the latest queued request and wait for `finished`, including a skipped reveal's pending DOM update. Queued choices cannot write `data-theme`, local storage or the cookie. I test actual palette actions before the update, during the animation and after a skip, plus rapid choices across both controls.
- I want theme changes to retain their smooth circular reveal. I save the choice before starting a view transition, then synchronously commit the selected swatch with the page colours. I disable nested CSS transitions during the reveal, and only the active transition can clear its styles. My regression checks cover snapshot timing, interrupted and skipped reveals, reduced motion and repeated fallback fades.
