...
export function initRockPaperScissors() {
  ...
  while (!username) {
    username = prompt('Enter a username for the leaderboard:')?.trim() ?? null;
  }
  localStorage.setItem(USER_KEY, username);

  // Tracks SESSION scores
  let scores: Scores = { playerWins: 0, cpuWins: 0, ties: 0 };
  ...
  function renderLeaderboard(data: LeaderboardEntry[]) {
  ...
  function normalizeThrow(throwStr: string) {
  ...
  function rpsLogic(playerThrow: string, cpuThrow: string) {
  ...
  function updateScoreDisplay() {
    ...
    document.getElementById('playerWins')!.textContent = String(scores.playerWins);
    document.getElementById('cpuWins')!.textContent = String(scores.cpuWins);
    document.getElementById('ties')!.textContent = String(scores.ties);
    document.getElementById('netScore')!.textContent = String(net);

    const displayHigh = lifetimeMaxNet === -Infinity ? '-' : String(lifetimeMaxNet);
    document.getElementById('highscore')!.textContent = displayHigh;
  }

  function handleThrow(playerThrow: string) {
  ...
  document.querySelectorAll('.throw').forEach((btn) => {
    btn.addEventListener('click', () => {
      handleThrow((btn as HTMLElement).dataset.throw!);
    });
  });
